...
public class RobotDesktop extends Robot implements IRobot {
	...
	private void doType(KeyMode mode, int... keyCodes) {
		...
		// if first key is shift we might need to press accented character
		else {
			int startIndex=0;

			if(keyCodes.length>=3 && keyCodes[0]==KeyEvent.VK_SHIFT){
				System.out.println("1="+keyCodes[0]);
				System.out.println("2="+keyCodes[1]);
				doKeyPress(keyCodes[0]);
				doKeyPress(keyCodes[1]);
				//delay(20);
				doKeyRelease(keyCodes[0]);
				doKeyRelease(keyCodes[1]);
				startIndex=2;
				//delay(20);
				
			}

			for (int i = startIndex; i < keyCodes.length; i++) {
//				if(i>=0) {
//				
//					System.out.println((i+1)+"="+keyCodes[i]);
//				}
				doKeyPress(keyCodes[i]);
			}
			for (int i = startIndex; i < keyCodes.length; i++) {
